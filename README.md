...
> **「3 门禁」** = 流程中带 ★ 的三关：**G3** Spec Lock、**G4** Analyze、**G7** Converge。  
> 完整门禁 **G1～G8**（无 G6）见 [`docs/名词解析.md`](./docs/名词解析.md) § 三 与 `templates/feature/context.md`。

| 步骤 | 名称 | 门禁 |
|------|------|------|
| 1～2 | 需求澄清（建议 grill）→ PRD + 技术方案 | G1、G2（可选确认） |
| **2.5 ★** | **Spec Lock**（你确认清单） | **G3** |
| 3～4 | 规范 → 任务拆解 | — |
| **2.6 ★** | **Analyze**（实现前） | **G4** |
| 5 | 开发 + 单测 + review（规范轴） | G5（Hook 派生） |
| **5.5 ★** | **Converge**（规格轴，可循环） | **G7** |
| 5.6 | 踩坑回流 FDK（可选） | — |
| **6** | 接口 / 文档 / 提交 | **G8 交付确认** |
| — | **UI 走查**（结构复核 P0 + 视觉 P1） | 结构项纳入 G7 |
| — | grill / diagnose / handoff（按需） | 不进门禁 |
...
- kit: **1.7.1**（见 [CHANGELOG.md](./CHANGELOG.md)）
